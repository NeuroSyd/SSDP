...
def set_random_seed(seed):
    ...
    torch.cuda.manual_seed(seed)        # 设置PyTorch的GPU随机种子（单GPU）
    torch.cuda.manual_seed_all(seed)    # 设置PyTorch的GPU随机种子（多GPU）
    torch.backends.cudnn.deterministic = True  # 确保CUDA的卷积操作是确定性的
    torch.backends.cudnn.benchmark = False     # 禁用CUDA的自动调优
...
# 定义Dataset类
class FashionMNIST_SNN_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_sequence = torch.stack(img_sequence, dim=0)  # [seq_length, 784]
...
def plot_spike_activity(spike_sequence, num_neurons=100):
    """
    可视化脉冲活动
    spike_sequence: [batch_size, seq_length, hidden_dim]
    """
    # 选择前batch_size=1的样本
    spikes = spike_sequence[0].cpu().numpy()  # [seq_length, hidden_dim]
    # 选择前num_neurons个神经元
    ...
    # 使用imshow绘制热力图
    img = plt.imshow(spikes.T, aspect='auto', cmap='Greys', interpolation='nearest')

    # 设置轴标签和标题，并调整字体大小
    plt.xlabel('Time Steps', fontsize=18)
    plt.ylabel('Neurons', fontsize=18)
    plt.title('Spike Activity Heatmap', fontsize=20, fontweight='bold')

    # 添加颜色条并设置其标签和字体大小
    cbar = plt.colorbar(img)
    cbar.set_label('Spiking', fontsize=16)
...
def main():
    ...
    # 超参数
    input_dim = 28 * 28  # Fashion-MNIST的输入维度
    hidden_dim = 1024  #
    output_dim = 10  # Fashion-MNIST的输出维度
    batch_size = 256  #
    num_epochs = 100
    learning_rate = 0.01
    ...
    transform = transforms.Compose([
        transforms.ToTensor(),  # 将图像转换为张量 [1, 28, 28]，范围 [0,1]
        # 可以添加更多预处理步骤，如归一化
        # transforms.Normalize((0.5,), (0.5,))
    ])

    # 加载Fashion-MNIST数据集
    train_dataset = FashionMNIST_SNN_Dataset(train=True, transform=transform, seq_length=seq_length)
    valid_dataset = FashionMNIST_SNN_Dataset(train=False, transform=transform, seq_length=seq_length)
    test_dataset = FashionMNIST_SNN_Dataset(train=False, transform=transform, seq_length=seq_length)
    ...
    # 新增：保存验证损失到文件的路径
    val_loss_save_path = 'val_loss_log.txt'  # 可改为.csv/.npy
    ...
    for epoch in range(num_epochs):
        ...
        model.train()
        running_loss = 0.0
        running_fr = 0.0  # 新增：记录每个epoch的总脉冲发放率
        for batch_idx, (inputs, labels) in enumerate(tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs}")):
            ...
            running_fr += firing_rate.item()  # 累加脉冲发放率

            if (batch_idx + 1) % 50 == 0:
                avg_loss = running_loss / 50
                avg_fr = running_fr / 50  # 计算平均脉冲发放率
                print(f'Epoch [{epoch + 1}/{num_epochs}], Step [{batch_idx + 1}/{len(train_loader)}], '
                      f'Loss: {avg_loss:.4f}, Firing Rate: {avg_fr:.4f}, '
                      f'Firing Rate Loss: {loss_fr.item():.4f}')
                running_loss = 0.0
                running_fr = 0.0  # 重置累加器
        # 记录每个epoch的损失和脉冲发放率
        # 这里使用训练集来计算平均损失和发放率
        model.eval()
        epoch_loss = 0.0
        ...
        with torch.no_grad():
            for inputs, labels in train_loader:
                ...
                epoch_fr += firing_rate.item()

        avg_epoch_loss = epoch_loss / len(train_loader)
        avg_epoch_fr = epoch_fr / len(train_loader)
        loss_history.append(avg_epoch_loss)
        fr_history.append(avg_epoch_fr)
        print(
            f'==> Epoch [{epoch + 1}/{num_epochs}] 完成: 平均损失: {avg_epoch_loss:.4f}, 平均脉冲发放率: {avg_epoch_fr:.4f}')
        ...
        # 追加写入每个epoch的验证集loss
        with open(val_loss_save_path, 'a') as f:
            f.write(f"{epoch + 1},{avg_val_loss:.6f}\n")

        print(f'==> 验证集: 平均损失: {avg_val_loss:.4f}, 准确率: {val_accuracy:.2f}%')



        scheduler.step()

    # 新增：绘制损失曲线和脉冲发放率曲线在同一个图中
    def plot_loss_and_fr(loss_history, fr_history, num_epochs):
        ...
        plt.rcParams.update({
            ...
            'figure.titlesize': 22  # 整个图的标题字体大小
        })

        # 第一个Y轴：损失曲线
        color = 'tab:blue'
        ax1.set_xlabel('Epoch', fontsize=18)  # 设置x轴标签字体大小
        ax1.set_ylabel('Loss', color=color, fontsize=18)  # 设置y轴标签字体大小
        ax1.plot(epochs, loss_history, 'b-', label='Training Loss', marker='o', markersize=6)
        ax1.tick_params(axis='y', labelcolor=color, which='major', labelsize=16)  # 设置主刻度字体大小
        ax1.legend(loc='upper left', fontsize=16)  # 设置图例字体大小

        # 第二个Y轴：脉冲发放率
        ax2 = ax1.twinx()  # 创建第二个y轴
        color = 'tab:red'
        ax2.set_ylabel('Spiking Rate', color=color, fontsize=18)  # 设置y轴标签字体大小
        ax2.plot(epochs, fr_history, 'r-', label='Spiking Rate', marker='x', markersize=6)
        ax2.tick_params(axis='y', labelcolor=color, which='major', labelsize=16)  # 设置主刻度字体大小
        ax2.legend(loc='upper right', fontsize=16)  # 设置图例字体大小

        # 设置标题字体大小
        plt.title('Loss and Spiking Rate', fontsize=20, fontweight='bold')
        ...
        # 设置 y 轴刻度更精细
        # 计算y轴的最小值和最大值
        min_loss = min(min(loss_history),
                       min(loss_history))  # 可能需要修正为 min_loss = min(min(loss_history), min(loss_without_ssdp))
        max_loss = max(max(loss_history),
                       max(loss_history))  # 同上，修正为 max_loss = max(max(loss_history), max(loss_without_ssdp))

        # 定义主刻度和次刻度
        ax1.set_yticks(np.arange(0, max_loss + 0.05, 0.05))  # 主刻度每0.05
        ax1.minorticks_on()
        ax1.tick_params(axis='y', which='minor', length=4, color='gray', labelsize=16)  # 设置次刻度字体大小

        ax2.minorticks_on()
        ax2.tick_params(axis='y', which='minor', length=4, color='gray', labelsize=16)  # 设置次刻度字体大小
    ...
    def test_classifier(model, test_loader):
        ...
        correct = 0
        total = 0
        all_spikes = None  # 新增：用于保存脉冲活动以供可视化
        with torch.no_grad():
        ...
        if all_spikes is not None:
            plot_spike_activity(all_spikes, num_neurons=100)  # 可调整可视化的神经元数量
